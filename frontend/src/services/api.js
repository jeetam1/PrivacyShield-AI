    ...
    async (config) => {
        ...
        if (token) {
            try {
                ...
                // If the current token is expired, clean out storage and redirect
                if (decoded.exp < currentTime) {
                    localStorage.removeItem('access_token');
                    localStorage.removeItem('refresh_token');
                    window.location.href = '/login';
                ...
                // Attach SimpleJWT compatible Bearer token
                config.headers.Authorization = `Bearer ${token}`;
            } catch (error) {
                console.error("Token structural tracking validation fault:", error);
            }
...
// 3. Centralized Django API Endpoints
// Notice the trailing slashes at the end of every string path—crucial for DRF!
export const scannerService = {
    // Auth endpoints
    ...
    // File Management & Scanner endpoints
    upload: (formData) => API.post('scanner/upload/', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
    }),
    getHistory: () => API.get('scanner/history/'),
